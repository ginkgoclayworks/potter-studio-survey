...
#     - You choose whether to share your studio name (optional)
...
#             value=data.get('studio_name', ''),
#             help="Leave blank to remain anonymous"
#         )
...
#         if location_state == "Other/Non-US":
#             location_other = st.text_input(
#                 "Please specify location:",
#                 value=data.get('location_other', '')
#             )
#             data['location_other'] = location_other
    
#     with col2:
#         space_sqft = st.number_input(
#             "Total usable studio space (sq ft) *",
#             min_value=100, max_value=50000, step=50,
#             value=data.get('space_sqft', 100),
#             help="Member-accessible space only"
#         )
#         data['space_sqft'] = space_sqft
...
#              "Small town/rural",
#              "Mixed use district"],
#             index=["Urban core (downtown)", "Urban neighborhood", "Suburban", 
#                    "Small town/rural", "Mixed use district"].index(data.get('area_type', 'Suburban')) 
#                    if data.get('area_type') in ["Urban core (downtown)", "Urban neighborhood", 
#                                                 "Suburban", "Small town/rural", "Mixed use district"] else 2
#         )
...
#              "500,000-1M",
#              "Over 1M"],
#             index=["Under 50,000", "50,000-100,000", "100,000-250,000", 
#                    "250,000-500,000", "500,000-1M", "Over 1M"].index(data.get('metro_population', 'Under 50,000'))
#                    if data.get('metro_population') in ["Under 50,000", "50,000-100,000", "100,000-250,000",
#                                                        "250,000-500,000", "500,000-1M", "Over 1M"] else 0
#         )
...
#                         ["Electric cone 6", "Electric cone 10", "Gas", "Wood", "Other"],
#                         index=["Electric cone 6", "Electric cone 10", "Gas", "Wood", "Other"].index(kilns[i].get('type', 'Electric cone 6')) if kilns[i].get('type') in ["Electric cone 6", "Electric cone 10", "Gas", "Wood", "Other"] else 0,
#                         key=f"k_type_{i}"
#                     )
#                     kilns[i]['type'] = kiln_type
#                 with col2:
#                     kiln_size = st.selectbox("Size (cu ft)",
#                         ["Small (<7)", "Medium (7-12)", "Large (12-18)", "XL (18+)"],
#                         index=["Small (<7)", "Medium (7-12)", "Large (12-18)", "XL (18+)"].index(kilns[i].get('size', 'Small (<7)')) if kilns[i].get('size') in ["Small (<7)", "Medium (7-12)", "Large (12-18)", "XL (18+)"] else 0,
#                         key=f"k_size_{i}"
#                     )
...
#     try:
#         _access_index = access_options.index(_saved_access) if _saved_access else None
#     except ValueError:
#         _access_index = None
...
#                  "Ongoing/drop-in",
#                  "Mix"],
#                 index=["One-time workshop", "Multi-week series (4-12 weeks/semesters)", 
#                        "Ongoing/drop-in", "Mix"].index(data.get('class_format', 'Multi-week series (4-8 weeks)'))
#                        if data.get('class_format') in ["One-time workshop", "Multi-week series (4-8 weeks)", 
#                                                         "Ongoing/drop-in", "Mix"] else 1
#             )
...
#         try:
#             _profit_index = profit_options.index(_saved_profit) if _saved_profit else None
#         except ValueError:
#             _profit_index = None
...
#         try:
#             _crowding_index = crowding_options.index(_saved_crowding) if _saved_crowding else None
#         except ValueError:
#             _crowding_index = None
...
#     if has_waitlist and waitlist_length > 0:
...
#         try:
#             _pricing_index = pricing_options.index(_saved_pricing) if _saved_pricing else None
#         except ValueError:
#             _pricing_index = None
...
#         try:
#             _expand_index = expand_options.index(_saved_expand) if _saved_expand else None
#         except ValueError:
#             _expand_index = None
...
#         try:
#             _raise_index = raise_prices_options.index(_saved_raise) if _saved_raise else None
#         except ValueError:
#             _raise_index = None
...
#     try:
#         _status_index = status_options.index(_saved_status) if _saved_status else None
#     except ValueError:
#         _status_index = None
...
#             try:
#                 _closure_index = closure_options.index(_saved_closure) if _saved_closure else 0
...
#     try:
#         _impact_index = impact_options.index(_saved_impact) if _saved_impact else None
#     except ValueError:
#         _impact_index = None
