...
#     # Derived metrics
#     df["sqft_per_member"] = df["space_sqft"] / df["current_members"]
#     df["members_per_wheel"] = df["current_members"] / df["total_wheels"]
#     df["rent_per_member"] = df["rent"] / df["current_members"]
#     df["rent_per_sqft_annual"] = (df["rent"] * 12) / df["space_sqft"]
...
#     st.header("🎨 Free Survey Insights")
...
#         median_sqft = df["sqft_per_member"].median()
...
#         median_ratio = df["members_per_wheel"].median()
...
#     # NEW: Rent affordability
#     st.subheader("💰 Rent Affordability")
#     col1, col2 = st.columns(2)
    
#     with col1:
#         st.markdown("**Rent per Member**")
#         median_rent_per_member = df["rent_per_member"].median()
...
#         median_rent_sqft = df["rent_per_sqft_annual"].median()
...
#         with col1:
#             st.metric("25th Percentile", f"${df['tier1_price'].quantile(0.25):.0f}")
#         with col2:
#             st.metric("Median", f"${df['tier1_price'].median():.0f}")
#         with col3:
#             st.metric("75th Percentile", f"${df['tier1_price'].quantile(0.75):.0f}")
...
#         staffing_counts["Has Staff"] = staffing_counts["Has Staff"].map({
#             True: "Has Paid Staff",
#             False: "No Paid Staff"
#         })
...
#         # Normalize boolean-like text to 1/0 first to avoid "str / int" TypeError
#         def _to_bool01(s):
...
#         # Use non-null responses as denominator (falls back to total if none missing)
...
#                     df_buildout["buildout_cost_total"] / df_buildout["space_sqft"]
#                 )
#                 median_per_sqft = df_buildout["buildout_per_sqft"].median()
